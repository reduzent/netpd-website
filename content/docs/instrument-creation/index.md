...
are the modules keeping the state of each parameter used by your instrument
...
instrument) name as second argument. Some take even more arguments. Read more
about all the available [netpd-abstractions](../netpd-abstractions).
...
of the slider (see second image abot).  Both, the send and receive are named
...
usage. Different data types require different ways to interact with them. However,
some principles apply to all types. More complex data types provide both, methods
for data retrieval and methods for data manipulation. Please note that operations
